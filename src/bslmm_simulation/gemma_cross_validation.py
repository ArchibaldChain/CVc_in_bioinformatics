...
def bslmm_train_test(geno_tr: pd.DataFrame,
    ...
    # check if bslmm_sigma has nan
    if np.any(np.isnan(bslmm_sigma)):
        warnings.warn(
            f'There is nan in bslmm_sigma: {bslmm_sigma} and s_a={s_a}, s_b={s_b}, s_e={s_e}, gor.pi={gor.pi}\n'
            + 'changing bslmm_sigma to sigmas')
        bslmm_sigma = sigmas
