...
def load(file) -> np.ndarray:
    ...
    data_drop_gene_selected = data_drop.sample(10_000, axis=0)
    SNP = data_drop_gene_selected.values.T
    return SNP


def phenotype_simulator(SNPs, num_large_effect, large_effect, small_effect):
    ## get the Beta
    d = num_large_effect
...
def cross_validation_correction(SNPs: np.ndarray,
    ...
    K_relatedness = 1 / np.sqrt(sc) * W_tr @ W_tr.T
    ...
    ### WLS
    H_cv_wls_k = getHcv_for_Kfolds(X_tr, y_tr, H_function_wls, nfolds=10, V=V)
...
def cross_validation_simulation(SNP_file='data/SNP_in_200GENE_chr1.csv',
    ...
    for i in range(simulation_times):
        ...
        lmm, wls, ols, ridge = cross_validation_correction(
            SNPs, y, num_fixed_snps)
